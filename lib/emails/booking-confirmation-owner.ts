interface BookingConfirmationOwnerProps {
  ownerName: string;
  dogName: string;
  salonName: string;
  serviceName: string;
...
export function bookingConfirmationOwnerEmail(p: BookingConfirmationOwnerProps): {
  subject: string;
  html: string;
  text: string;
} {
  const subject = `Booking confirmed at ${p.salonName} 🐾`;
  const dateStr = formatDate(p.scheduledAt);
  const timeStr = formatTime(p.scheduledAt);

  const addressLine = p.address
    ? `<tr><td style="padding:6px 0;font-size:14px;color:#95a5a6;">${p.address}</td></tr>`
    : "";
...
<body style="margin:0;padding:0;background:#f9f8f4;font-family:Helvetica,Arial,sans-serif;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
        ...
        <tr><td style="background:#ffffff;border-radius:20px;padding:36px 32px;box-shadow:0 2px 12px rgba(44,62,80,0.07);">
          ...
          <div style="text-align:center;margin-bottom:24px;">
            <a href="${p.appUrl}/dashboard/owner" style="display:inline-block;background:#eae45c;color:#2c3e50;font-weight:700;font-size:15px;padding:14px 32px;border-radius:100px;text-decoration:none;">
  ...
  const text = `Hi ${p.ownerName},\n\n${p.dogName}'s appointment at ${p.salonName} is confirmed!\n\nService: ${p.serviceName}\nDate: ${dateStr}\nTime: ${timeStr}\n${p.address ? `Address: ${p.address}\n` : ""}\nView your appointments: ${p.appUrl}/dashboard/owner\n\nPowered by Groomr`;
