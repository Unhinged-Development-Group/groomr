...
| Cloudinary URL transforms | Insert transformation params after `/upload/` in the URL. `b_f9f8f4` (bare hex) is invalid — hex needs `b_rgb:f9f8f4` but the colon breaks Next.js image URL parsing. Use named colors (`b_white`) or avoid background params. Cover/profile photo transforms live in `lib/utils.ts` and both use `c_fill` so Cloudinary outputs exact fixed dimensions (cover 1200×720 via `g_auto`; profile 500×500 via `g_face`) regardless of upload aspect ratio — display containers use CSS `object-cover`, which is a no-op once the output shape is fixed. `c_fill` crops to fill (no letterboxing); use `c_fit` only when you must show the whole image within bounds. Both transforms prepend `b_whitesmoke` (closest CSS-named colour to brand cream `#f9f8f4`) so transparent-PNG uploads — groomers often use a logo — flatten onto a near-cream backing instead of showing black/through. **Cover and profile use the same delivery-layer model**: both upload the original file untouched (`handleCoverPhotoUpload`/`handleProfileImageUpload` in `ProfileEditor.tsx` POST the raw `File` straight to Cloudinary) and all sizing/cropping/flattening happens in the URL transform. There is **no** client-side canvas resize — an earlier `resizeToSquare` helper was removed because exporting to JPEG destroyed alpha and baked black behind transparent logos before upload. Note: any pre-existing profile JPEGs uploaded before that removal already have black baked in (real pixels, not transparency) and must be re-uploaded — no transform recovers them. |
| `VerificationDocs` fallback literal | `profile-editor.ts` ~line 148 has a **second** `VerificationDocs` object literal (early-return when no groomer profile exists). Adding fields to the `VerificationDocs` type requires updating **both** this fallback and the main one at the bottom of `loadProfileEditorData`. Local build passes; Vercel catches it. |
