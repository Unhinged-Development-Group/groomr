...
| Cloudinary URL transforms | Insert transformation params after `/upload/` in the URL. `b_f9f8f4` (bare hex) is invalid — hex needs `b_rgb:f9f8f4` but the colon breaks Next.js image URL parsing. Use named colors (`b_white`) or avoid background params. Cover/profile photo transforms live in `lib/utils.ts` and both use `c_fill` so Cloudinary outputs exact fixed dimensions (cover 1200×720 via `g_auto`; profile 500×500 via `g_face`) regardless of upload aspect ratio — display containers use CSS `object-cover`, which is a no-op once the output shape is fixed. `c_fill` crops to fill (no letterboxing, so no background param needed); use `c_fit` only when you must show the whole image within bounds. |
