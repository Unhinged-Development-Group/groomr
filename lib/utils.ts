...
export function toCoverPhotoUrl(url: string | null | undefined): string | null {
  ...
  // Leading e_trim crops uniform/transparent margins first so a padded logo's
  // artwork fills the frame; on a normal photo (no uniform border) it's a no-op.
  return url.replace("/upload/", "/upload/e_trim/b_whitesmoke,c_fill,g_auto,w_1200,h_720,q_auto,f_auto/");
}
...
export function toProfilePhotoUrl(url: string | null | undefined): string | null {
  ...
  // Leading e_trim crops uniform/transparent margins first so a padded logo's
  // artwork fills the circle; on a normal photo (no uniform border) it's a no-op.
  return url.replace("/upload/", "/upload/e_trim/b_whitesmoke,c_fill,g_face,w_500,h_500,q_auto:good,f_auto/");
}
