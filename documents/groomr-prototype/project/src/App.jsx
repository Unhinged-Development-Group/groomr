...
function App() {
  ...
  const role = tw.userRole || "customer"; // 'customer' | 'groomer'

  React.useEffect(() => {
    if (tw.loggedIn && !user) {
      setUser(role === "groomer" ? { name: "Lola García", role: "groomer" } : { name: "Anya Patel", role: "customer" });
    }
  ...
  const onMessages = () => nav("messages");

  // Login routes to the right dashboard for the active role
  const completeAuth = () => {
    setAuthModal(null);
    const u = role === "groomer" ? { name: "Lola García", role: "groomer" } : { name: "Anya Patel", role: "customer" };
    setUser(u);
    setTweak("loggedIn", true);
    nav(role === "groomer" ? "groomer-dashboard" : "customer");
    setToast(role === "groomer" ? "Welcome back, Lola." : "Welcome to Groomr.");
  };
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        {route === "landing" && (
          <Landing onSearch={onSearch} onSignup={onSignup} onForGroomers={() => nav("for-groomers")}/>
        )}
        ...
        {route === "booking" && groomer && service && (
          <Booking groomer={groomer} service={service} onConfirm={onConfirm} onBack={() => nav("profile")}/>
        )}
        {route === "confirmation" && (
        ...
        {route === "customer" && user && (
          <CustomerDashboard user={user} onView={onView}
            onSearch={() => nav("search")} onBook={(g, s) => onBook(g, s)}/>
        )}
        {route === "groomer-dashboard" && user && (
          <GroomerDashboard user={user} onLogout={onLogout} onNavMessages={onMessages}/>
        )}
        {route === "messages" && user && (
          <Messages user={user} mode={user.role}/>
        )}
        {route === "for-groomers" && <ForGroomers onRegister={() => nav("register")}/>}
        {route === "register" && (
      ...
      <Modal open={!!authModal} onClose={() => setAuthModal(null)}>
        <div className="space-y-5">
          <div>
            ...
            <div className="flex items-center gap-2 mt-4 bg-white border border-pebble-grey/20 rounded-full p-1.5 w-fit">
              {[["customer","I'm a dog owner"],["groomer","I'm a groomer"]].map(([k,l]) => (
                <button key={k} onClick={() => setTweak("userRole", k)}
                  className={`px-4 py-1.5 rounded-full text-xs font-bold transition-colors focus-ring ${role===k?"bg-deep-slate text-alabaster-cream":"text-deep-slate hover:bg-pebble-grey/10"}`}>{l}</button>
              ))}
            </div>
          </div>
      ...
      <TweaksPanel>
        <TweakSection label="Demo state">
          ...
          <TweakRadio label="Role" value={role}
            options={["customer","groomer"]}
            onChange={(v) => {
              setTweak("userRole", v);
              if (tw.loggedIn) {
                setUser(v === "groomer" ? { name: "Lola García", role: "groomer" } : { name: "Anya Patel", role: "customer" });
                nav(v === "groomer" ? "groomer-dashboard" : "customer");
              }
            }}/>
        </TweakSection>
        ...
        <TweakSection label="Jump to screen">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, padding: "6px 12px 10px" }}>
            {[
              ...
              ["Customer", () => { setUser({ name: "Anya Patel", role: "customer" }); setTweak("loggedIn", true); setTweak("userRole", "customer"); nav("customer"); }],
              ["Groomer dash", () => { setUser({ name: "Lola García", role: "groomer" }); setTweak("loggedIn", true); setTweak("userRole", "groomer"); nav("groomer-dashboard"); }],
              ["Messages", () => { if (!user) { setUser({ name: "Anya Patel", role: "customer" }); setTweak("loggedIn", true); } nav("messages"); }],
