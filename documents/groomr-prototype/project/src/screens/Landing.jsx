// Landing — Search-first hero, two parallel tracks (customer + groomer), trust strip, testimonials.

const TRUST_LOGOS = ["The Doghouse", "Wuff Mag", "Pawscene", "London Pet Co.", "Bark Times"];

const HOW_IT_WORKS = [
{ Icon: SearchIcon, title: "Find your match", body: "Search by postcode. See real availability, prices, and reviews from neighbours." },
{ Icon: CalendarIcon, title: "Book in seconds", body: "No back-and-forth texts. Pick a slot, pay, done." },
{ Icon: HeartIcon, title: "Build a routine", body: <>Rebook your favourite groomer in two taps. <span className="tagline">"Your dog deserves a regular."</span></> }];


const TESTIMONIALS = [
{ name: "Anya", dog: "Lab cross", text: "I used to forget to book until Marlow was a tangled mess. Groomr just… does it for me." },
{ name: "Tom", dog: "Bichon", text: "Finding a groomer who's good with anxious dogs was impossible. Found two within 1 mile." },
{ name: "Priya", dog: "Yorkie · 13yrs", text: "Senior-care filter is a quiet miracle. Bramble actually enjoys it now." }];


const Landing = ({ onSearch, onSignup, onForGroomers }) => {
    ...
    <div className="page-fade">
        ...
        <div className="w-full px-6 lg:px-12 xl:px-20 pt-16 pb-20 md:pt-24 md:pb-32 grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="space-y-7 max-w-xl">
            ...
            <h1 className="font-fredoka text-5xl md:text-6xl lg:text-7xl text-deep-slate leading-[0.95]">
              Book your dog's<br />next groom in<br /><span className="relative inline-block">
                <span className="relative z-10">minutes.</span>
                <span className="absolute inset-x-0 bottom-1 h-4 bg-groomr-gold/60 -z-0 rounded-sm"></span>
              </span>
            </h1>
            <p className="text-lg md:text-xl text-pebble-grey font-nunito leading-relaxed" style={{ fontSize: "28px" }}>
              <span className="tagline text-deep-slate">"Your dog deserves a regular."</span>
            </p>

            <div className="pt-2">
              <SearchPill value={q} onChange={setQ} onSubmit={onSearch}
              placeholder="Postcode, town, or 'near me'"
              ctaLabel="Find Groomers" />
              <div className="flex flex-wrap gap-2 mt-3 text-xs">
                ...
                <button key={t} onClick={() => {setQ(t);onSearch(t);}}
                className="bg-white border border-pebble-grey/20 hover:border-deep-slate transition-colors text-deep-slate font-bold px-3 py-1 rounded-full focus-ring">
                    {t}
                  </button>
                )}
              </div>
            ...
            <div className="flex items-center gap-6 pt-3 text-sm font-bold text-pebble-grey">
              <div className="flex items-center gap-2"><ShieldIcon size={20} /> Verified groomers</div>
              <div className="flex items-center gap-2"><CalendarIcon size={20} /> Real-time booking</div>
            </div>
          </div>
          ...
          <div className="relative h-[480px] md:h-[560px] hidden lg:block">
            <div className="absolute top-0 right-0 w-72 h-80 rounded-[24px] overflow-hidden shadow-lift rotate-[3deg]">
              <img src="https://images.unsplash.com/photo-1583512603805-3cc6b41f3edb?auto=format&fit=crop&w=600&q=70" alt="" className="w-full h-full object-cover" />
            </div>
            <div className="absolute bottom-12 left-0 w-64 h-72 rounded-[24px] overflow-hidden shadow-lift -rotate-[4deg]">
              <img src="https://images.unsplash.com/photo-1591768793355-74d04bb6608f?auto=format&fit=crop&w=600&q=70" alt="" className="w-full h-full object-cover" />
            </div>
            <div className="absolute top-32 left-44 w-60 rounded-[20px] bg-white p-5 shadow-modal border border-pebble-grey/20 rotate-[2deg]">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-full bg-groomr-gold flex items-center justify-center font-fredoka text-deep-slate">M</div>
                <div>
                  <p className="font-fredoka text-sm text-deep-slate leading-tight">Marlow's next groom</p>
      ...
      <section className="w-full px-6 lg:px-12 xl:px-20 py-20 md:py-28">
        <div className="text-center max-w-2xl mx-auto mb-14 space-y-3">
          ...
          <h2 className="font-fredoka text-4xl md:text-5xl text-deep-slate">Booking grooming, finally easy.</h2>
        </div>
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {HOW_IT_WORKS.map(({ Icon, title, body }, i) =>
          <div key={title} className="bg-white border border-pebble-grey/20 rounded-[20px] p-7 card-lift">
      ...
      <section className="bg-alabaster-cream border-y border-pebble-grey/15 py-20 md:py-24">
        <div className="w-full px-6 lg:px-12 xl:px-20 grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-5">
            <h2 className="font-fredoka text-4xl md:text-5xl text-deep-slate">Spend less time admin-ing. More time grooming.</h2>
            <p className="text-pebble-grey text-lg leading-relaxed font-nunito">
              Online booking, automatic reminders, payments, and a profile that shows up where dog owners actually look.
              Free to list. No commission on existing clients — ever.
            </p>
            <div className="flex flex-wrap gap-4 pt-2">
              <button onClick={onForGroomers} className="btn-primary font-nunito font-bold px-7 py-3 rounded-full focus-ring shadow-subtle">
                Become a Groomr
              </button>
            </div>
            <div className="flex items-center gap-8 pt-5 border-t border-pebble-grey/20 mt-6">
              ...
              <div><p className="font-fredoka text-3xl text-deep-slate">38hrs</p><p className="text-xs font-bold text-sage-leaf uppercase tracking-wider">Saved monthly avg</p></div>
      ...
      <section className="w-full px-6 lg:px-12 xl:px-20 py-20 md:py-28">
        <div className="text-center max-w-2xl mx-auto mb-14 space-y-3">
          ...
          <h2 className="font-fredoka text-4xl md:text-5xl text-deep-slate">Real dogs. Real regulars.</h2>
        </div>
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {TESTIMONIALS.map((t, i) =>
          <figure key={i} className="bg-white border border-pebble-grey/20 rounded-[20px] p-7 card-lift relative">
              ...
              <figcaption className="mt-5 pt-5 border-t border-pebble-grey/10 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-sage-leaf text-white font-fredoka flex items-center justify-center">{t.name.charAt(0)}</div>
                <div>
