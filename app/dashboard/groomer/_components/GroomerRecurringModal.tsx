...
import { createGroomerRecurringSeries } from "@/app/actions/recurring";
...
export function GroomerRecurringModal({
  appointmentId,
  dogName,
  serviceName,
  scheduledAt,
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await createGroomerRecurringSeries({
        appointmentId,
        frequency,
      ...
      setDoneCount(result.appointmentsCreated);
      setDone(true);
    });
  }
    ...
    <Modal open onClose={onClose} size="sm">
      {done ? (
        <div className="text-center space-y-4 py-4">
          ...
          <h2 className="font-fredoka text-2xl text-deep-slate">Recurring series set up!</h2>
          <p className="text-sm text-pebble-grey font-nunito">
            {doneCount} appointment{doneCount !== 1 ? "s" : ""} have been added to your calendar.
          </p>
          <button
            onClick={() => { onCreated(doneCount); onClose(); }}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Day of week */}
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-pebble-grey mb-2">Day</p>
          ...
          {!hasEndDate && (
            <p className="text-xs text-pebble-grey font-nunito">
              Ongoing — creates a rolling 6-month window of appointments.
            </p>
          )}
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button type="submit" disabled={isPending} className="flex-1 btn-primary font-nunito font-bold py-3 rounded-full text-sm focus-ring shadow-subtle disabled:opacity-50">
              {isPending ? "Creating…" : "Set up recurring"}
            </button>
          </div>
        </form>
      )}
